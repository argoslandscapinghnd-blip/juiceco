...
interface Props {
  usuario: string;
  onUsuarios: () => void; onSucursales: () => void; onBebidas: () => void;
  onInsumos: () => void; onUnidades: () => void; onRecetas: () => void;
  onReportes: () => void; onModoCajero: () => void; onCerrarSesion: () => void;
}
...
export default function AdminMenuScreen({
  usuario, onUsuarios, onSucursales, onBebidas, onInsumos, onUnidades,
  onRecetas, onReportes, onModoCajero, onCerrarSesion,
}: Props) {
  const handlers: Record<string, () => void> = {
    usuarios: onUsuarios, sucursales: onSucursales, bebidas: onBebidas,
    unidades: onUnidades, insumos: onInsumos, recetas: onRecetas, reportes: onReportes,
  };
  ...
  return (
    <section>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <Logo size="normal" />
        <button onClick={onCerrarSesion} style={{ background: "none", border: "none", color: colors.danger, cursor: "pointer", fontSize: 13, fontWeight: "bold" }}>Cerrar sesión</button>
      </div>
      ...
      {modulos.map((m) => (
        <button key={m.key} onClick={handlers[m.key]} style={{ ...cardStyle, width: "100%", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", border: "none", textAlign: "left", marginBottom: 10 }}>
          <span style={{ fontSize: 28, width: 52, height: 52, borderRadius: 12, background: colors.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{m.emoji}</span>
          <div>
