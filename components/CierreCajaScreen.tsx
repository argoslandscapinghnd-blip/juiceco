...
export default function CierreCajaScreen({
  ...
  const totalContado  = BILLETES.reduce((s, d) => s + (parseFloat(cantidades[d] || "0") || 0) * d, 0);
  ...
  const handleCerrar = async () => {
    ...
    await supabase.from("sesiones_caja").update({
      activa: false, cerrada_en: new Date().toISOString(),
      fondo_final: totalContado, diferencia, observacion: observacion || null,
    }).eq("id", sesionCajaId);
    setCerrando(false);
    onCerrado();
  };
  ...
  return (
    <section>
      ...
      <div style={{ ...cardStyle, marginBottom: 14 }}>
        ...
        {/* Encabezado */}
        <div style={{ display: "grid", gridTemplateColumns: "70px 1fr 90px", gap: 8, marginBottom: 6 }}>
          <span style={{ fontSize: 12, fontWeight: "bold", color: colors.textMuted }}>Valor</span>
          <span style={{ fontSize: 12, fontWeight: "bold", color: colors.textMuted, textAlign: "center" }}>Cantidad</span>
          <span style={{ fontSize: 12, fontWeight: "bold", color: colors.textMuted, textAlign: "right" }}>Subtotal</span>
        </div>
        ...
        {BILLETES.map((d) => {
          ...
          return (
            <div key={d} style={{ display: "grid", gridTemplateColumns: "70px 1fr 90px", gap: 8, alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontWeight: "bold", fontSize: 14, color: colors.textPrimary }}>L {d}</span>
              <input
                type="number" min="0" placeholder="0"
                value={cantidades[d] ?? ""}
                onChange={(e) => setCantidades(prev => ({ ...prev, [d]: e.target.value }))}
                style={{ padding: "8px 10px", borderRadius: 8, border: `1px solid ${colors.border}`, fontSize: 14, textAlign: "center" }}
              />
              <span style={{ fontSize: 13, textAlign: "right", fontWeight: subtotal > 0 ? "bold" : "normal", color: subtotal > 0 ? colors.primary : colors.textMuted }}>
                {subtotal > 0 ? `L ${fmt(subtotal)}` : "—"}
              </span>
            </div>
          );
        })}
