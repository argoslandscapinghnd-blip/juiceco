...
interface VentaSucursal {
  ...
  total: number;
  num_ventas: number;
  cajeros: string[]; // usuarios activos ese período en esa sucursal
}

interface VentaCajero {
  usuario_id: string;
  nombre: string;
  total: number;
  num_ventas: number;
  sucursales: string[]; // sucursales donde operó ese período
}
...
// ─── Date Range Picker (inline, simple) ───────────────────────────────────────
...
export default function DashboardScreen({ onBack }: { onBack: () => void }) {
  ...
  const getRange = useCallback((): { from: string; to: string } => {
    ...
    return { from: first, to: toDateStr(last) };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: ventas } = await supabase
        .from("ventas")
        .select("id, total, metodo_pago, sucursal_id, usuario_id, creada_en")
        .gte("creada_en", fromTs)
        .lte("creada_en", toTs);
      ...
      const utilidadEstimada = totalVentas * 0.36; // ~36% margen estimado
      ...
      // 5. Ventas por sucursal (+ cajeros únicos de ese período en esa sucursal)
      const sucAgg: Record<string, { total: number; num: number; usuarios: Set<string> }> = {};
      ventas.forEach((v) => {
        if (!sucAgg[v.sucursal_id]) {
          sucAgg[v.sucursal_id] = { total: 0, num: 0, usuarios: new Set() };
        }
      ...
      const sucList: VentaSucursal[] = Object.entries(sucAgg)
        .map(([id, d]) => ({
          sucursal_id: id,
          nombre: sucMap[id] || id,
          total: d.total,
      ...
      // 6. Ventas por cajero (+ sucursales únicas)
      const usrAgg: Record<string, { total: number; num: number; sucursales: Set<string> }> = {};
      ventas.forEach((v) => {
        if (!v.usuario_id) return;
        if (!usrAgg[v.usuario_id]) {
          usrAgg[v.usuario_id] = { total: 0, num: 0, sucursales: new Set() };
        }
      ...
      const cajList: VentaCajero[] = Object.entries(usrAgg)
        .map(([id, d]) => ({
          usuario_id: id,
          nombre: usrMap[id] || id,
          total: d.total,
      ...
      const prodList: VentaProducto[] = Object.entries(prodAgg)
      ...
      const metList: VentaMetodo[] = Object.entries(metAgg)
        .map(([metodo_pago, d]) => ({ metodo_pago, ...d }))
        .sort((a, b) => b.total - a.total);
      setMetodos(metList);
        ...
        >
          ...
          {!loading && (
            <span style={{ fontSize: 10, color: "#9ca3af" }}>
              ↺ {lastUpdate.toLocaleTimeString("es-HN", { hour: "2-digit", minute: "2-digit" })}
            </span>
          )}
        </div>
          ...
          <>
              ...
              {sucursales.map((s) => (
                <div key={s.sucursal_id} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", margin: "4px 0" }}>
                    <Bar value={s.total} max={maxSuc} />
                  </div>
                  {/* Meta info: ventas + cajeros */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            <Section title="Por cajero" icon="👤">
              ...
              {cajeros.map((c) => (
                <div key={c.usuario_id} style={{ marginBottom: 12 }}>
                  ...
                  {/* Meta info: ventas + promedio + sucursales */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginTop: 2 }}>
                    <span style={{ fontSize: 11, color: "#6b7280" }}>
                      {c.num_ventas} ventas · prom. {fmt(c.total / c.num_ventas)}
                    </span>
            ...
            <Section title="Por producto" icon="🥤">
              ...
              {productos.map((p, i) => (
                <div key={p.nombre_producto} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <span style={{ fontSize: 14, marginRight: 6, minWidth: 20 }}>
                      {medals[i] || "  "}
                    </span>
                    <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: "#111827" }}>
                      {p.nombre_producto}
                  ...
                  <div style={{ display: "flex", alignItems: "center", marginTop: 4, paddingLeft: 26 }}>
                    <Bar value={p.cantidad} max={maxProd} />
                  </div>
                </div>
              ))}
            </Section>
            ...
            >
              ...
              <span style={{ fontSize: 12, color: "#6b7280" }}>
                Resumen general de ventas, unidades, ticket promedio y utilidad.
              </span>
            </div>
          </>
        )}
